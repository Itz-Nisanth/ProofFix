// ProofFix End-to-End Smoke Test Suite
import { calculateDeterministicPriority } from './src/lib/priorityEngine.js';
import { evaluateDuplicateCandidate, calculateDistanceMetres } from './src/lib/duplicateEngine.js';
import { getIncidents, getIncidentById, createIncident, findOpenCandidatesWithinRadius, addConfirmationToIncident, addResolutionToIncident } from './src/lib/db.js';
import { analyzeIncidentPhoto, compareVisualEvidence } from './src/lib/gemini.js';
...
async function runSmokeTest() {
  ...
  function record(stepNum, stepName, status, details = '') {
  ...
  try {
    ...
    // 12. Create incident in Supabase / PostGIS store
    ...
    const fetchedInc = await getIncidentById(createdIncident.id);
    if (fetchedInc) {
      record(13, 'Verify PostGIS spatial point & query', 'PASS', `Spatial Point: Point(${fetchedInc.longitude}, ${fetchedInc.latitude}) verified.`);
    } else {
    ...
    // 15. Verify Stage 1 retrieves nearby candidate
    const stage1Candidates = await findOpenCandidatesWithinRadius(secondReportLocation.lat, secondReportLocation.lng, 35);
    if (stage1Candidates.length > 0) {
      record(15, 'Stage 1: PostGIS ST_DWithin candidate search (~30m)', 'PASS', `Found ${stage1Candidates.length} open candidate incident(s) within 35m`);
    } else {
      record(15, 'Stage 1: PostGIS ST_DWithin candidate search (~30m)', 'FAIL');
    }
    ...
    const updatedWithConf = await addConfirmationToIncident(createdIncident.id, {
      latitude: secondReportLocation.lat,
      longitude: secondReportLocation.lng,
      location_accuracy: 4.2,
      captured_at: new Date().toISOString()
    });
    if (updatedWithConf && updatedWithConf.confirmation_count === prevCount + 1) {
      record(18, 'Increment independent confirmation count', 'PASS', `Confirmations incremented from ${prevCount} → ${updatedWithConf.confirmation_count} on incident ${createdIncident.id}`);
    } else {
      record(18, 'Increment independent confirmation count', 'FAIL');
    }

    // 19. Test city filter on Risks
    const chennaiRisks = await getIncidents({ city: 'Chennai' });
    const puducherryRisks = await getIncidents({ city: 'Puducherry' });
    record(19, 'Test city filter on Risks', 'PASS', `Chennai: ${chennaiRisks.length} incidents, Puducherry: ${puducherryRisks.length} incidents`);
    ...
    const resolutionPhotoPartial = 'https://lh3.googleusercontent.com/aida/AEtjO1XvEFaovTcC4klwvoRBqgp-0xePaLkGXVVD7iqRdnzyCILuOPyBLHh1kRqrR5sNu8ai_7Qk5Uf2Wkl3MJd0aZG0xKhewUxLMzt13dvx9ZiJL6BTNuzF1m8Cmk9W7UbM119aia_cZU6w43PYjQYk779VQwfMknbzdwWhn1Ng6LO0q3ZHDQefFEJqfBaGzog3vrSW3uCPdNJQbNDzystP4Rjg8BkY6hHdI9jflFHTf1Z2AJxPo88xdlhoXEsS';
    ...
    record(24, 'Run before/after Gemini visual comparison', 'PASS', `Comparison verified: ${verifyJson.is_verified}. Explanation: "${verifyJson.resolution?.verification_notes || 'Verified'}"`);
    ...
    console.log('\n====================================================');
    console.log(`TEST SUITE COMPLETED: ${results.filter(r => r.status === 'PASS').length}/${results.length} PASSED`);
    console.log('====================================================\n');
  } catch (error) {
